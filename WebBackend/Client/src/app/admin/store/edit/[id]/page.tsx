...
export default function EditStoreItem() {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const res = await api.get(`/store/items/${params.id}`);
        setFormData({
          name: res.data.name,
          description: res.data.description,
          price: res.data.price,
          gameItemId: res.data.gameItemId,
          category: res.data.category || '',
          isActive: res.data.isActive,
          isUnique: res.data.isUnique !== undefined ? res.data.isUnique : true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.put(`/store/items/${params.id}`, { ...formData, id: params.id });
      router.push('/admin/store');
      router.refresh();
    } catch (error: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-white">编辑商品</h1>
      </div>
